...
| `modules/<name>/api/` (bean `implements SecurityRuleCustomizer`) | **Từ 2026-09-16**: rule phân quyền theo endpoint (`.permitAll()`/`.hasRole(...)`/`.hasAnyRole(...)`) của module đặt ở đây, ngay cạnh Controller - KHÔNG sửa `base.security.SecurityAutoConfiguration#securityFilterChain` nữa (xem `BASE_FRAMEWORK_GUIDE.md` mục 10.4, và `AGENT.md` Điều 9). `securityFilterChain` tự động gom mọi bean `SecurityRuleCustomizer` trong context. | `UserSecurityRules.java` |
| `modules/<name>/infrastructure/security/` | Hiếm khi cần: chỉ dùng khi module phải override **toàn bộ** hành vi mặc định của `base.security` (ví dụ tự cung cấp một `SecurityFilterChain` bean khác hẳn, thay vì chỉ thêm rule cho path của mình - trường hợp đó dùng `SecurityRuleCustomizer` ở trên). | *(hiếm dùng, hiện chưa có ví dụ trong dự án)* |
| `modules/<name>/infrastructure/security/` (implement `base.security.CredentialAuthenticator`) | Cầu nối xác thực username/password sang dữ liệu thật của module - trả principal xác thực được cho `base.security.api.AuthCtl` dùng khi đăng nhập ở chế độ `STANDALONE`. Đặt ở `infrastructure/` (không phải `application/`) vì đây là adapter hiện thực một port do tầng ngoài (`base.security`) định nghĩa, gọi thẳng `Repository` + `PasswordEncoder` của module - cùng bản chất với một Repository implementation, không điều phối use case hay quản lý transaction (đã thống nhất 2026-09-16, trước đó code đặt tạm ở `application/`). | `UserCredentialAuthenticator.java` |
...
| `docs/` | Chứa toàn bộ tài liệu quy chuẩn kiến trúc, API và prompt mẫu của dự án. Tài liệu API riêng của mỗi module nghiệp vụ nằm trong thư mục con `docs/<ten_module>/` (xem `AGENT.md` Điều 11). | `MICROSERVICE_ARCHITECTURE_GUIDE.md`, `BASE_FRAMEWORK_GUIDE.md`, `LANGUAGE_API_GUIDE.md`, `PROMPT_TEMPLATES.md`, `user/USER_API_GUIDE.md` |
...
7. **Bước 7 (Phân Quyền)**: Tạo `ProductSecurityRules implements SecurityRuleCustomizer` (annotate `@Component`) trong `modules/product/api/`, khai báo rule cho `/public/product/**` trong đó - **không** sửa `SecurityAutoConfiguration` trong `base` (xem `BASE_FRAMEWORK_GUIDE.md` mục 10.4). Bỏ qua bước này không làm hỏng gì (mặc định `anyRequest().authenticated()` vẫn áp dụng), nhưng endpoint mới sẽ chỉ yêu cầu đăng nhập chứ không giới hạn đúng role mong muốn.
...
> 8. **Phân Quyền Endpoint Mới (bắt buộc, xem `BASE_FRAMEWORK_GUIDE.md` mục 10)**: mọi endpoint mới phải có rule tường minh (`.permitAll()` nếu công khai, `.hasRole(...)`/`.hasAnyRole(...)` nếu giới hạn theo vai trò) khai báo qua một bean `@Component implements SecurityRuleCustomizer` đặt trong `modules/<module_name>/api/` của chính module đó - **tuyệt đối không sửa `base.security.SecurityAutoConfiguration`** để thêm rule này (vi phạm Điều 2). Không dựa vào default `anyRequest().authenticated()` để "đoán" ý định phân quyền.
