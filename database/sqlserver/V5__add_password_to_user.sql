...
-- never uses this column at all (that mode's auth service owns credentials entirely). SQL Server has
-- no "ADD COLUMN IF NOT EXISTS" form, so guarded the same way V2/V3/V4 guard their own DDL.
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('tbl_user') AND name = 'password')
ALTER TABLE tbl_user ADD password NVARCHAR(100);
